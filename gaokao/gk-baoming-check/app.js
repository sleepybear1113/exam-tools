(function () {
    ...
    const CHINA_PLACE_PREFIXES_2D = [
        ["浙江", "杭州", "宁波", "温州", "嘉兴", "湖州", "绍兴", "金华", "衢州", "舟山", "台州", "丽水", "嘉善县", "海宁市", "平湖市", "桐乡市", "海盐县"],
        ["北京"],
        ["天津"],
        ...
        ["重庆", "梁平县", "城口县", "丰都县", "垫江县", "武隆县", "忠县", "云阳县", "奉节县", "巫山县", "巫溪县", "石柱", "秀山", "酉阳", "彭水"],
    ...
    const CHINA_PLACE_PREFIXES_FLAT = CHINA_PLACE_PREFIXES_2D.flat();

    // 计算每个字符串的长度
    const CHINA_PLACE_PREFIXES_FLAT_LENGTH = CHINA_PLACE_PREFIXES_FLAT.map(s => s.length);
    const CHINA_PLACE_PREFIXES_LENGTH_RANGE = [...new Set(CHINA_PLACE_PREFIXES_FLAT_LENGTH)].sort((a, b) => a - b);

    const CHINA_PLACE_PREFIXES_OBJ = {};

    CHINA_PLACE_PREFIXES_FLAT.forEach(s => {
        CHINA_PLACE_PREFIXES_OBJ[s] = true;
    });
    ...
    function validateRecord(r) {
        ...
        if (addr) {
            ...
            for (let i = 0; i < CHINA_PLACE_PREFIXES_LENGTH_RANGE.length; i++) {
                const len = CHINA_PLACE_PREFIXES_LENGTH_RANGE[i];
                if (addr.length < len) continue;

                const prefix = addr.substring(0, len);
                if (CHINA_PLACE_PREFIXES_OBJ[prefix]) {
                    foundProvince = true;
                    break;
                }
            }

            if (!foundProvince) tips.push('邮寄地址格式需有省市');
        }
    ...
    async function handlePdf(file) {
        ...
        try {
            ...
            records = pages.map(p => parsePageTextToRecord(p));
